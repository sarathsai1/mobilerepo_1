...
const EmployeesList: React.FC<EmployeesListProps> = ({ id, name, phoneNumber, expertise }) => {
    ...
    const handleDelete = async () => {
        try {
            ...
            await axios.delete(`http://54.152.49.191:8080/employee/${id}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
        ...
        <View style={styles.container}>
            <View style={styles.cardContent}>
                <View style={styles.cardInnerContent}>
                    ...
                    <View>
                        <View style={styles.sideIcons}>
                            <TouchableOpacity onPress={() => { navigation.navigate("EditEmployee", { employeeId: id }) }}>
                                <Image
                                    source={require('../../../assets/icons/edit.png')}
                                    style={{ width: 30, height: 30 }}
                                />
